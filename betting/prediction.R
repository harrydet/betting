...
nsim = 100000
matches <- mapply(get_score, fixtures$HOME.TEAM, fixtures$AWAY.TEAM, nsim, SIMPLIFY = FALSE)
fixtures$H <- sapply(matches, function(x) x[1])
fixtures$D <- sapply(matches, function(x) x[2])
fixtures$A <- sapply(matches, function(x) x[3])


df_prediction <- fixtures %>%
